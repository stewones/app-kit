...
gulp.task('html', ['inject', 'partials'], function() {
    ...
    var assets;
    return gulp.src(path.join(conf.paths.tmp, '/serve/*.html')).pipe($.inject(partialsInjectFile, partialsInjectOptions)).pipe(assets = $.useref.assets()).pipe($.rev()).pipe(jsFilter).pipe($.ngAnnotate()).pipe($.uglify({
        ...
        quotes: true,
        conditionals: true
    })).pipe(htmlFilter.restore()).pipe(gulp.dest(path.join(conf.paths.dist, '/'))).pipe($.size({
...
})).pipe($.filter('**/*.{eot,svg,ttf,woff,woff2}')).pipe($.flatten()).pipe(gulp.dest(path.join(conf.paths.dist, '/fonts/')));
});
...
gulp.task('angular-charts-fonts', function() {
    return gulp.src(conf.paths.src + '/assets/icons/**/*.{eot,svg,ttf,woff,woff2}').pipe($.flatten()).pipe(gulp.dest(conf.paths.dist + '/fonts/'));
});
gulp.task('build-lib', ['lib-js', 'lib-js-min', 'lib-css']);
gulp.task('lib-js-min', [], function() {
    return gulp.src([
        ...
        preserveComments: $.uglifySaveLicense
    })).pipe($.size()).pipe(gulp.dest(path.join(conf.paths.build, '/')));
});
